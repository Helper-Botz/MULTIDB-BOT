...
@Client.on_message(filters.command('search_google'))
async def search_google(client, msg):
...
@Client.on_message(filters.command('search_stack'))
async def stack_search(client, msg):
    split = msg.text.split(None, 1)
    if len(split) == 1:
        return await msg.reply_text("**ɢɪᴠᴇ ǫᴜᴇʀʏ ᴛᴏ sᴇᴀʀᴄʜ**")
    to_del = await msg.reply_text("**sᴇᴀʀᴄʜɪɴɢ ᴏɴ ɢᴏᴏɢʟᴇ...**")
