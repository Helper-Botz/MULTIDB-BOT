...
from plugins.helpers.config import ADMINS, GIT_TOKEN # BOT_USERNAME
import httpx
...
@Client.on_message(filters.command('search_info'))
async def myinfo_command(client, message):
    ...
    if len(message.command) > 1:
        ...
        try:
            ...
            await client.send_message(chat_id=message.chat.id, text="𝖨𝖭𝖵𝖠𝖫𝖨𝖣 𝖴𝖲𝖤𝖱 𝖨𝖣.")
            return
...
# @Client.on_message(filters.command("leavegroup")& filters.user(OWNER_ID))
...
@Client.on_message(filters.command("search_repo"))
async def repo(client, message):
    async with httpx.AsyncClient() as client:
        response = await client.get("https://api.github.com/repos/Helper-Botz/NASRANI/contributors")
    ...
    if response.status_code == 200:
        ...
        text = f"""[𝖱𝖤𝖯𝖮 𝖫𝖨𝖭𝖪](https://github.com/Helper-Botz/NASRANI) | [𝖦𝖱𝖮𝖴𝖯](https://t.me/NASRANI_SUPPORT)
| 𝖢𝖮𝖭𝖳𝖱𝖨𝖡𝖴𝖳𝖮𝖱𝖲 |
----------------
{list_of_users}"""
        await message.reply_text(text=text, disable_web_page_preview=True)
    else:
        await client.send_message(message.chat.id, text="Failed to fetch contributors.")
# -------------------------------------------------------------------------------------------------------
# -------------------------------------------------------------------------------------------------------
...
# --------------------------------------------------------------------------------- #
#--------------------------------------------------------------------------
@Client.on_message(filters.command("search_id"))
...
@Client.on_message(filters.command(["search_github", "search_git"]))
