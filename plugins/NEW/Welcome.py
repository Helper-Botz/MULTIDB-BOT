...
def welcomepic(pic, user, chatname, id, uname):
    background = Image.open("plugins/helpers/dil.png")
    pfp = Image.open(pic).convert("RGBA")
    pfp = circle(pfp)
    pfp = pfp.resize((1157, 1158))
    draw = ImageDraw.Draw(background)
    font = ImageFont.truetype('plugins/helpers/font.ttf', size=110)
    welcome_font = ImageFont.truetype('plugins/helpers/font.ttf', size=60)
...
def spellcheck(pic, mv_rqst, user, chatname, id, uname):
    background = Image.open("plugins/helpers/check.png")
    pfp = Image.open(pic).convert("RGBA")
    pfp = circle(pfp)
    pfp = pfp.resize((1157, 1158))
    draw = ImageDraw.Draw(background)
    ...
    draw.text((1800, 700), f'NAME: {user}', fill=(255, 255, 255), font=font)
    draw.text((1800, 830), f'ID: {id}', fill=(255, 255, 255), font=font)
    draw.text((1800, 965), f"SEARCH : {mv_rqst}", fill=(255, 255, 255), font=font)
    pfp_position = (391, 336)
#    background.paste(pfp, pfp_position, pfp)
    background.save(f"{id}.png")
    return f"{id}.png"
...
async def greet_group(client, member: ChatMemberUpdated):
...
➖➖➖➖➖➖➖➖➖➖➖➖
""",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(f"⦿ ᴀᴅᴅ ᴍᴇ ⦿", url=f"https://t.me/nasrani_robot?startgroup=true")]])
        )
    except Exception as e:
        LOGGER.error(e)
    try:
...
async def bot_wel(client, message):
